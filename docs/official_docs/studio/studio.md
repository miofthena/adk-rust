...
```toml
# Only Gemini
adk-model = { version = "1.1.0", default-features = false, features = ["gemini"] }

# Mixed providers (e.g., Gemini + Anthropic)
adk-model = { version = "1.1.0", default-features = false, features = ["gemini", "anthropic"] }

# Ollama only (no API key needed)
adk-model = { version = "1.1.0", default-features = false, features = ["ollama"] }
```
